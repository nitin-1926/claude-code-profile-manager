...
export function TurnView({
  ...
  const blocks = turn.blocks.filter((b) => (b.kind === 'thinking' ? showThinking : true))
      ...
      <div className="space-y-1.5">
        {blocks.map((b, i) => (
          <BlockView
            key={i}
            block={b}
            blockIndex={turn.blocks.indexOf(b)}
            profile={profile}
            sessionId={sessionId}
            turnUuid={turn.uuid ?? ''}
            forceOpen={isTarget || expandAll}
          />
        ))}
      </div>
    </div>
...
function ToolChip({
  ...
  async function toggle() {
    const next = !open
    setOpen(next)
    if (!next || body || !turnUuid) return
    setLoading(true)
    try {
      setBody(await api.history.toolBody(profile, sessionId, turnUuid, blockIndex))
    } catch {
      setBody({ body: '', fullBytes: 0, truncated: false })
    } finally {
      setLoading(false)
    }
  }
