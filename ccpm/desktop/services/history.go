...
type HistoryService struct {
	mu sync.Mutex
	// cancels maps a caller-supplied search token to its run.
	//
	// The value is a POINTER so a run can tell whether the entry under its
	// token is still its own. Tokens are generated by the frontend and are
	// meant to be unique; if one is ever reused, the first run to finish used
	// to delete the second's cancel func on its way out, leaving the second
	// uncancellable and a later CancelSearch falling through to the tombstone
	// branch — where it would cancel a THIRD search that had scanned nothing.
	// Identity comparison makes a reused token merely useless rather than
	// actively wrong.
	cancels map[string]*searchRun
...
// searchRun is one in-flight scan. Compared by pointer identity, which is why
// it exists at all — Go cannot compare two context.CancelFunc values.
type searchRun struct{ cancel context.CancelFunc }
...
//
// It reads the usage store but never syncs it. usage.Sync rewrites all three
// store files unconditionally, so syncing here would emit a change event on
// every History fetch. UsageService.Get already owns the sync; History is a
// reader.
//
// BuildIndex below DOES write, to <profileDir>/usage/history.json, and that
// path sits in the watched tree. It writes only when a transcript actually
// changed, and watcher.go excludes the file by name for precisely this reason —
// without that exclusion an append to any transcript produced a write, a
// ccpm:changed event, a refetch, and a second full scan.
//
...
func resolveTranscriptPath(profile, sessionID, relPath string) (string, error) {
	...
	e, ok := allowTranscript(transcript.LoadIndex(dir), sessionID, relPath)
	if !ok {
		// The allowlist comes from the sidecar; search enumerates the
		// filesystem live. Nothing rebuilds the index in between, so a hit in a
		// transcript written since the History tab last listed — a subagent
		// file a running session just spawned, or an entirely new session — is
		// legitimately absent from it. The watcher deliberately skips
		// projects/, so the stale window never closes on its own, and the user
		// saw a blank reader with no error.
		//
		// Rebuild once and re-ask. The build is incremental, so the cost is a
		// stat per transcript, and a genuinely bogus path pays it too — which
		// is acceptable: it is a rescan of the caller's own profile, not an
		// amplification.
		if _, err := transcript.BuildIndex(dir); err != nil {
			return "", err
		}
		e, ok = allowTranscript(transcript.LoadIndex(dir), sessionID, relPath)
	}
	if !ok {
		return "", fmt.Errorf("no transcript on record for session %s", sessionID)
	}
	want := e.RelPath
	if relPath != "" {
		want = relPath
	...
	if fi.Mode()&fs.ModeSymlink != 0 {
		return "", errors.New("refusing to open a transcript path that is a symlink")
	}
	return full, nil
}

// allowTranscript checks sessionID and relPath against what the index records,
// returning the entry when the pair is permitted.
//
// Split out so the caller can retry it against a freshly built index without
// duplicating the rule.
func allowTranscript(ix *transcript.Index, sessionID, relPath string) (*transcript.Entry, bool) {
	e := ix.Entries[sessionID]
	if e == nil || e.RelPath == "" {
		return nil, false
	}
	if relPath != "" && !slices.Contains(e.SubPaths, relPath) && relPath != e.RelPath {
		return nil, false
	}
	return e, true
}
...
func (s *HistoryService) Search(profile, query, token string, includeToolResults bool) (transcript.SearchResult, error) {
	...
	if token != "" {
		// A token already in flight means the frontend reused one. Cancel the
		// older run rather than orphaning it: it can no longer be reached by
		// CancelSearch, so leaving it would let a superseded full-profile scan
		// run to completion.
		if prev := s.cancels[token]; prev != nil {
			prev.cancel()
		}
		s.cancels[token] = run
	}
	...
	defer func() {
		s.mu.Lock()
		// Only if the entry is still ours — see the field comment.
		if token != "" && s.cancels[token] == run {
