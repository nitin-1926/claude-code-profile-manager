...
`ccpm statusline` is the in-TUI status line for a launched session — it shows **which profile is running** plus usage/limits at the bottom of the Claude Code window. It reads Claude Code's status JSON on stdin and renders **two rows** (Claude Code displays each printed line as its own row): row 1 is where you are — `⬢ work · claude-code-profile-manager/ccpm · ⎇ feat/history-tab` (profile, repo with the subdirectory you are in, git branch); row 2 is what it is costing — `Opus 5 · ctx 34% · effort high · 5h 42% ↺16:15 · 7d 12% ↺Mon 08:25 · $1.23` (model, context usage, reasoning effort, the usage windows, session cost). Splitting them keeps the identity row still while the numbers tick. The `5h`/`7d` segments are the **percentage used** of the rolling subscription usage windows — matching Claude's own `/usage` panel (Pro/Max accounts only; they appear after the first API response and are absent for API-key profiles). A reset falling on today's date shows a bare clock; one on another date names the weekday, or gives a date once it is more than a week out. That is a calendar-day rule, not a distance rule — an evening five-hour window that resets after midnight will name tomorrow's weekday even though it is only hours away. The `effort` segment appears only for models that report a reasoning-effort level. Every segment drops out when its data is absent, and a row with nothing to say is not printed at all — so a session with no profile and no workspace resolved prints only the usage row, and a profile with no model or cost yet prints only the identity row. The two rows never merge: the profile is always on row 1 and the model always on row 2, so an API-key profile outside a git repo still prints two rows (`⬢ work` / `Opus 4.8 · $0.12`). The branch is read from `.git/HEAD` directly rather than by shelling out to git, because the status line re-runs on every assistant message. The line is **ANSI-coloured** by default — the `5h`/`7d` percentage shades green→amber→red as remaining headroom shrinks; set `NO_COLOR` for plain text. The `$…` segment is the API-equivalent cost estimate of the session, not a subscription charge. `ccpm run` injects this automatically as the profile's `statusLine` when the profile has none of its own — it never overwrites a statusLine you set in `~/.claude/settings.json`, a profile, or a trusted project. Turn the auto-injection off persistently with `ccpm config set statusline false`, per-launch with `ccpm run <profile> --no-statusline`, and remove an already-injected one with `ccpm settings statusline "" --profile <name>`. You don't normally call `ccpm statusline` yourself; Claude Code invokes it.
