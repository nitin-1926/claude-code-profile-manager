...
In the **desktop app**, the same thing lives in the **Settings** tab under "Status line": a row per segment with an Off / Row 1 / Row 2 selector, a live preview of both rows, and a scope switch between the global default and this profile's override. It writes by shelling out to `ccpm statusline configure`, so the desktop app needs a `ccpm` CLI new enough to have that command. You don't normally call `ccpm statusline` yourself; Claude Code invokes it.
