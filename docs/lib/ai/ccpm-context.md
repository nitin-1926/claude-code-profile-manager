...
- **Build/run**: requires the Wails CLI (`go install github.com/wailsapp/wails/v2/cmd/wails@latest`), then `cd ccpm && make desktop` produces `desktop/build/bin/CCPM.app`; `make desktop-dev` runs a hot-reload window. The app is **ad-hoc signed / unsigned** for now, so first launch on macOS needs a one-time Gatekeeper bypass (right-click → Open).
