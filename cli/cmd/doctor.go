...
func runDoctor(cmd *cobra.Command, args []string) error {
	...
	// -----------------------------------------------------------------
	// Exit code: only real issues fail; warnings never do.
	// -----------------------------------------------------------------
	if issues > 0 {
...
// checkSymlinkIntegrity inspects each profile's skills directory and reports
// any dangling symlinks. Copies are tolerated (Windows fallback) — we can't
// distinguish "copy on purpose" from "stale copy" without hashing against the
// share store every time, which would be expensive.
func checkSymlinkIntegrity(profileDirs map[string]string) []string {
	...
	for name, dir := range profileDirs {
		...
		for _, e := range entries {
			...
			if err != nil {
				issues = append(issues, fmt.Sprintf("%s/skills/%s: cannot stat (%v)", name, e.Name(), err))
				continue
			}
			...
			if _, err := filepath.EvalSymlinks(path); err != nil {
				issues = append(issues, fmt.Sprintf("%s/skills/%s: broken symlink (%v)", name, e.Name(), err))
			}
		}
	}
	sort.Strings(issues)
