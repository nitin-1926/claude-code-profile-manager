...
func EnsureDirs() error {
	dirs := []func() (string, error){Dir, SkillsDir, MCPDir, SettingsDir}
	for _, fn := range dirs {
		d, err := fn()
		if err != nil {
			return err
...
func Link(src, dst string) error {
	...
	// / the next `ccpm sync` can warn about degraded dedup.
	emitWindowsCopyFallbackWarning()
	if err := markWindowsCopyFallback(); err != nil {
		// Non-fatal; just a hint file.
		_ = err
	}
